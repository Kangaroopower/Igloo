...
$(function () {
	...
	var getToken = function(module, targ, url, callback, failure, extra) {
		...
		// verification
		if (module.match(/(rollback|userrights|undelete)/)) specialToken(module, targ);
		...
		switch (module) {
			case 'rollback':
				...
				ajax('get', rbtURL, function (data) {
					token = encodeURIComponent(data.query.pages[data.query.pageids[0]].revisions[0].rollbacktoken);
					ajax("post", url, token, callback, failure, extra);
				});
				break;
			case 'undelete':
				...
				ajax('get', detURL, function (data) {
					token = encodeURIComponent(data.query.deletedrevs[0].token);
					ajax("post", url, token, callback, failure, extra);
				});
					break;
			case 'userrights':
				...
				ajax('get', urURL, function (data) {
					token = encodeURIComponent(data.query.users[0].userrightstoken);
					ajax("post", url, token, callback, failure, extra);
				});
				break;
			default:
